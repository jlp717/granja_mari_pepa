...
async function obtenerIVARepercutido(fechaInicio, fechaFin, codigoCliente) {
  try {
    ...
    // Fuente oficial: CFC (cabecera de factura). CAC agrupa por albarán y omite series
    // directas (O/Q/C/E/T) que no pasan por albarán. Totales fiscales del ERP.
    const query = `
      SELECT
        TRIM(CFC.SERIEFACTURA) as SERIEFACTURA,
        CFC.NUMEROFACTURA,
        ...
        TRIM(CLI.NIF) as CIFCLIENTE,
        CFC.IMPORTEBASEIMPONIBLE as BASE_IMPONIBLE,
        CFC.IMPORTEIVA as IVA,
        COALESCE(CFC.IMPORTERECARGO, 0) as RECARGO,
        CFC.IMPORTETOTAL as TOTAL
      FROM DSEDAC.CFC CFC
      INNER JOIN DSEDAC.CLI CLI ON TRIM(CFC.CODIGOCLIENTE) = TRIM(CLI.CODIGOCLIENTE)
      WHERE (CFC.ANODOCUMENTO * 10000 + CFC.MESDOCUMENTO * 100 + CFC.DIADOCUMENTO) >= ?
        AND (CFC.ANODOCUMENTO * 10000 + CFC.MESDOCUMENTO * 100 + CFC.DIADOCUMENTO) <= ?
        AND CFC.NUMEROFACTURA > 0
        AND CFC.NUMEROFACTURA < 900000
        ${codigoCliente ? `AND TRIM(CFC.CODIGOCLIENTE) IN (${codigosInClause})` : ''}
        AND CFC.IMPORTEBASEIMPONIBLE <> 0
        AND CFC.IMPORTETOTAL <> 0
      ORDER BY ANOFACTURA DESC, MESFACTURA DESC, DIAFACTURA DESC,
               TRIM(CFC.SERIEFACTURA), CFC.NUMEROFACTURA
    `;
    ...
    logger.info('📊 IVA Repercutido obtenido (CFC oficial)', {
