    ...
    const query = `
      WITH FacturasBase AS (
        SELECT
          ...
          -- FIX 2026-04-06: Antes se hacía BASE * PORCENTAJE lo cual era incorrecto
          SUM(CAC.IMPORTEIVA1 + CAC.IMPORTEIVA2 + CAC.IMPORTEIVA3 + CAC.IMPORTEIVA4 + CAC.IMPORTEIVA5) AS IVA,

          -- Total: USAR IMPORTETOTAL del ERP directamente
          SUM(CAC.IMPORTETOTAL) AS TOTAL,
          
          MAX(COALESCE(CAC.IMPORTECOBRADOPENDIENTE, 0)) AS PENDIENTE,
          CAST(
            CASE
              WHEN MAX(CAC.DIAFACTURA) < 10 THEN '0' || TRIM(CAST(MAX(CAC.DIAFACTURA) AS CHAR(2)))
              ELSE TRIM(CAST(MAX(CAC.DIAFACTURA) AS CHAR(2)))
            END || '/' ||
            CASE
              WHEN MAX(CAC.MESFACTURA) < 10 THEN '0' || TRIM(CAST(MAX(CAC.MESFACTURA) AS CHAR(2)))
              ELSE TRIM(CAST(MAX(CAC.MESFACTURA) AS CHAR(2)))
            END || '/' ||
