...
async function forceRebuild() {
    try {
        ...
        // 1. Try Drop View Explicitly
        console.log("Dropping View...");
        try {
            await db.query("DROP VIEW JAVIER.V_MEDIOS_POWERBI");
            console.log("View Dropped.");
        } catch (e) {
            console.log("Warning dropping View (might be locked/missing):", e.message);
        }

        // 2. Create NEW UDTF (V2) to avoid conflicts with V1
        // Using VARCHAR for robustness (Safety Logic)
        const createFunc = `
            ...
            RETURN 
                ...
                FROM DSEDAC.MEDL1
        `;
        // Using OR REPLACE if supported, or ignore "Already Exists" if V2 was partly made
        try {
            await db.query(createFunc);
            console.log("UDTF V2 Created.");
        } catch (e) {
            console.log("Note on UDTF:", e.message);
            // If manual create needed
        }

        // 3. Recreate View pointing to V2
        ...
        console.log("✅ Success! View Updated to use V2.");
