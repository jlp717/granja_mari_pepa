...
const EXPECTED_ENERO_CLIENTES = 293;
const CONTADO_CLIENT_CODE = '4300005000';

// Expresión para detectar el negocio real (si es contado, usar el cliente del albarán)
...
async function run() {
    ...
    `);
      ...
      WHERE CAC.ANODOCUMENTO = ${EJERCICIO}
    ...
    `);
        ...
        const totalClientesEnero = enero[0]?.TOTAL_CLIENTES || 0;
        console.log(`\n   >>> ESPERADO:  ${EXPECTED_ENERO_CAJAS} cajas en Enero`);
        console.log(`   >>> OBTENIDO:  ${totalCajasEnero} cajas en Enero`);
        console.log(`   >>> CLIENTES:  ${totalClientesEnero} (Esperado: ${EXPECTED_ENERO_CLIENTES})`);
    ...
    `);
        ...
        console.log(`  Enero 2026 = ${EXPECTED_ENERO_CLIENTES} clientes:    ${totalClientesEnero === EXPECTED_ENERO_CLIENTES ? '✅' : '❌'} (obtenido: ${totalClientesEnero})`);
